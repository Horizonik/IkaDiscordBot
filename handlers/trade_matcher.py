...
from utils.constants import RESOURCE_EMOJIS, TRADE_REG_PATTERN, BOT_ENV
from utils.general_utils import create_embed


def load_trade_history():
    """load trade history from the last day"""
    return run_query(f"SELECT * FROM {BOT_ENV}_trades_history WHERE proposal_time >= NOW() - INTERVAL 1 DAY")


def save_trade_history(trade_history):
    """Save the trade history to the database."""
    run_query(f"""
        INSERT INTO {BOT_ENV}_trades_history (guild_id, user_id, offer, want, proposal_time)
        VALUES {", ".join(f"({guild_id}, {user_id}, '{trade['offer']}', '{trade['want']}', '{trade['timestamp']}')"
                          for guild_id, users in trade_history.items()
                          for user_id, trades in users.items()
                          for trade in trades)}
        ON DUPLICATE KEY UPDATE proposal_time = VALUES(proposal_time)
    """)
